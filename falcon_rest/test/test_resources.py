...
import falcon_rest
import pytest
from falcon_rest import testing

from falcon_rest.resources import TodoListResource, TodoResource
...
def test_get_todos(client):
    ...
    assert response.status == falcon_rest.HTTP_200
...
def test_create_todo(client):
    ...
    assert response.status == falcon_rest.HTTP_201
    data = json.loads(response.text)
...
def test_delete_todo(client):
    ...
    assert response.status == falcon_rest.HTTP_200
    response = client.simulate_get('/todos/1')
    assert response.status == falcon_rest.HTTP_404
